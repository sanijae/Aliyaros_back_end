...
export default function ProposalDetails() {
    ...
    const storeRef = ref(storage,`proposals/${params}`)

    useEffect(()=>{
        return(async()=>{
            try {
                const docRef = doc(db,'proposals',`${params}`)
    ...
    async function deleteDocs(title) {
        try {
            await deleteDoc(doc(db,'proposals',`${title}`))
            .then(async()=>{
                await deleteObject(storeRef).then(()=>window.location.pathname = '/Proposals')
