...
export default function TopicsDetails() {
    const [file,setFile] = useState('')
    const [message,setMessage] = useState('')
    const [error,setError] = useState('')
    const [show,setShow] = useState(false)
    const params = useParams().title
    const storeRef = ref(storage,`topics/${params}`)
    ...
    async function deleteDocs(title) {
        try {
            await deleteDoc(doc(db,'topics',`${title}`))
            .then(async()=>{
                await deleteObject(storeRef).then(()=>window.location.pathname = '/Topics')
